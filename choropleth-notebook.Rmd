...
In this session we will be using a R Markdown Notebook to document and share the script. R Markdown Notebooks are popular for reporting analyses as they can show the entire workflow from data manipulation to visualization, including what packages were used to maintain version control. This allows for a reproducible workflow.

First we will want to set the work directory. This will link R to the folder where your script and data are stored.
```{r}
...
Now, let's convert the shelter data frame into a simple feature of points using the sf package. Simple features are a standard for geopsatial features (i.e., points, lines and polygons). To learn more about the standard, check out: https://www.r-spatial.org/r/2016/02/15/simple-features-for-r.html. 
```{r}
...
In this case, we specified the coordinate reference system (crs) as 4326 (https://epsg.io/4326). 4326 assumes a ellipsoid surface, and is widely used for web mapping purposes.
...
To see your geospatial data on a simple map, we can simply plot it using base R.
...
To save space in memory, it's best to remove the plot afterwards.
...
```{r}
values <- gb_filtered$'Median Age' # the ratio/normalized value that will be visible on the map
...
```{r}
hist(values, main=paste("Distribution of", values_name), xlab=values_name)
```
...
tm_shape(gb_vancouver) +
  tm_fill(values_name, palette="BuPu", style="cont", n=6, title=values_name, id="name")+
  tm_borders(col="grey25", alpha=.5) +
...
Take your time and read tmap documentation to modify the look of the choropleth. Once you are happy with the visualization, you can export it as PDF or an image. You can also use the script within a R Shiny App.
